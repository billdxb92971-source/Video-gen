...
// Only construct the Gemini client if a key is present, so the app can run on fal.ai alone.
...
async function generateScriptGemini({ prompt, sceneCount, aspectRatio, tone }) {
  if (!ai) throw new Error("GEMINI_API_KEY is not set — required for the Gemini script provider");
...
async function generateSceneClip(scene, { aspectRatio, style, providerId }, jobDir) {
  ...
  return provider.generate(scene, { aspectRatio, style, ai }, jobDir);
}
